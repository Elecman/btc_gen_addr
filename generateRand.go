...
func main() {
	...
	for {
		...
		_, public := btcec.PrivKeyFromBytes(btcec.S256(), generateAddress())
		...
		if strings.HasPrefix(uaddr.EncodeAddress(), prefix) || strings.HasPrefix(caddr.EncodeAddress(), prefix) {
			...
			numFound++
			fmt.Printf("\nElapsed: %s\nUaddr: %s\nCaddr: %s\nwif: %x\nnumfound: %d\n",
				time.Since(beginTime), uaddr.EncodeAddress(), caddr.EncodeAddress(), generateAddress(),
				numFound)
			...
			if _, err := file.WriteString(uaddr.EncodeAddress() + " " + caddr.EncodeAddress() + " " + hex.EncodeToString(generateAddress()) + "\n"); err != nil {
				fmt.Printf("Error pushing data to file: %s", err)
				os.Exit(666)
			}
		}
